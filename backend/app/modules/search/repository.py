    ...
    async def search(self, workspace_id: str, term: str, limit: int) -> list[dict[str, Any]]:
        ...
        for collection, kind, field, extras in [
            ...
            cursor = self.db[collection].find({
                "workspace_id": workspace_id, field: match, **extras,
            }).sort([("updated_at", -1), ("_id", -1)]).limit(limit + 1).max_time_ms(2000)
        ...
        async for row in self.db.comments.aggregate(pipeline, maxTimeMS=2000):
            results.append({"kind": "ticket", "id": str(row["_id"]),
                            "title": row["body"][:240], "project_id": row["_page"]["project_id"],
        ...
        pipeline = [
            ...
            {"$unwind": "$person"}, {"$sort": {"person.name": 1, "_id": 1}},
            {"$limit": limit + 1},
        ]
        async for row in self.db.memberships.aggregate(pipeline, maxTimeMS=2000):
            results.append({"kind": "person", "id": str(row["person"]["_id"]),
