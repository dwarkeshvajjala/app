...
**Implementation notes (Milestone 7, `docs/tdr/0006`):** `comment.created`/`comment.updated` payloads carry an extra `project_id` field not present in `CommentOut`'s REST shape - WS-only, so a dashboard client holding multiple projects' comments in cache can tell which project's query a given event belongs to without an extra round trip. `presence.updated` is published only to `workspace:{id}:all` (staff-facing), never to a guest's own `project:{id}:client` channel - a guest reviewer never sees who else is reviewing the same page. Of the 7 event types above, `comment.recovery_updated`, `typing.*`, and `notification.new` are not yet emitted by anything (see TDR-0006 for why, and which milestone each is tied to).
