...
const PROJECT_TYPES: Array<{
  ...
  { id: "image", label: "Images", description: "PNG, JPG, WebP, GIF or SVG. Comments pin to the artwork itself.", available: true },
...
export function ProjectForm({ workspace, project, initialType, onClose }: {
  ...
  const [step, setStep] = useState<1 | 2>(project ? 2 : 1);
  ...
  function selectFiles(selected: File[]) {
    ...
    const allowed = type === "pdf"
      ...
      : selected.filter((file) => /^(image\/(png|jpeg|webp|gif|svg\+xml))$/.test(file.type) || /\.(png|jpe?g|webp|gif|svg)$/i.test(file.name));
    if (allowed.length !== selected.length) setFileError(type === "pdf" ? "Choose a valid PDF document." : "Choose PNG, JPG, WebP, GIF or SVG images.");
  ...
  if (done && created) {
    return (
      <Dialog title="Project created" onClose={onClose}>
  ...
  if (!project && step === 1) {
    return (
      <Dialog title="New project" onClose={onClose}>
        ...
        <div className="bl-project-type-body">
          <div className="bl-project-type-grid primary" role="radiogroup" aria-label="Project type">
            {PROJECT_TYPES.filter((item) => item.available).map((item) => (
              <button key={item.id} type="button" role="radio" aria-checked={typeChosen && type === item.id} className="bl-project-type-card" onClick={() => { setType(item.id as ProjectType); setTypeChosen(true); }}><span className="bl-type-icon"><TypeIcon type={item.id} /></span><span><strong>{item.label}</strong><small>{item.description}</small></span></button>
            ))}
          </div>
        ...
        <footer className="bl-dialog-actions bl-dialog-actions-bordered"><button type="button" className="bl-quiet" onClick={onClose}>Cancel</button><button type="button" className="bl-button mint" disabled={!typeChosen} onClick={() => setStep(2)}>Continue</button></footer>
      </Dialog>
    );
  }

  const canSubmit = Boolean(name.trim() && (client !== "new" || newClientName.trim()) && (type === "website" ? url.trim() : project || files.length > 0) && !fileError);

  return (
    <Dialog title={project ? "Project settings" : type === "website" ? "Add the page to review" : "Upload your files"} onClose={onClose}>
      ...
      <form className="bl-project-form" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
        ...
        <footer className="bl-dialog-actions bl-dialog-actions-in-form"><button type="button" className="bl-quiet" onClick={project ? onClose : () => setStep(1)}>{project ? "Cancel" : "Back"}</button><button className="bl-button mint" disabled={save.isPending || !canSubmit}>{save.isPending ? (uploaded.length ? `Uploading ${uploaded.length + 1} of ${files.length}…` : "Saving…") : project ? "Save changes" : created ? "Finish uploads" : "Create project"}</button></footer>
      </form>
    </Dialog>
  );
}
