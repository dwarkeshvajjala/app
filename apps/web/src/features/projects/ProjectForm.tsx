...
export function ProjectForm({ workspace, project, onClose }: { workspace: WorkspaceOut; project?: api.ProjectOut; onClose: () => void }) {
  ...
  const save = useMutation({ mutationFn: async () => {
    ...
    if (project) return api.updateProject(project.id, { name: name.trim(), ...(type === 'website' ? { target_origin: url.trim(), environment } : {}), client_id: clientId || null });
  ...
  return <Dialog title={project ? "Project settings" : "Create a project"} onClose={onClose}><form className="bl-form" onSubmit={(e) => { e.preventDefault(); save.mutate(); }}>
    ...
    {!project && type !== 'website' && <label>Files<input type="file" className="bl-input" required={files.length === 0} multiple={type === 'image'} accept={type === 'pdf' ? 'application/pdf' : 'image/png,image/jpeg,image/webp,image/gif'} onChange={(e) => setFiles(Array.from(e.target.files ?? []))} /><span className="bl-mono">{files.length} selected · {uploaded.length} uploaded</span></label>}
