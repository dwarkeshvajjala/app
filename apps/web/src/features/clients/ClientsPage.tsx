...
export function ClientsPage() {
  ...
  return <main className="bl-wrap"><header className="bl-head"><div><p className="bl-eyebrow">Workspace</p><h1>Clients</h1><p>Your client relationships and the projects you're reviewing together.</p></div><button className="bl-button" onClick={() => setEdit("new")}>＋ Add client</button></header>
    ...
    {!clients.isLoading && !clients.error && <div className="bl-table-wrap"><table className="bl-table"><thead><tr><th>Client</th><th>Main contact</th><th>Projects</th><th>Since</th><th>Actions</th></tr></thead><tbody>{visible.map((client) => <tr key={client.id}><td><button className="bl-text-button" onClick={() => setEdit(client)}><span className="bl-avatar">{client.name.slice(0, 2).toUpperCase()}</span><strong>{client.name}</strong></button></td><td>{client.contact_name || "No contact"}<small>{client.email || "No email"}</small></td><td><div className="bl-chip-row">{(projects.data ?? []).filter((p) => p.client_id === client.id && !p.archived_at).map((p) => <Link className="bl-chip" key={p.id} to={`/w/${workspace.slug}/p/${p.id}`}>{p.name}</Link>)}</div></td><td>{new Date(client.created_at).toLocaleDateString(undefined, { month: "short", year: "numeric" })}</td><td><button className="bl-quiet" onClick={() => setEdit(client)}>Edit</button><button className="bl-quiet" onClick={() => setArchive(client)}>Archive</button></td></tr>)}</tbody></table>{visible.length === 0 && <div className="bl-empty"><h2>{search ? "No clients match" : "Your next client starts here"}</h2><p>{search ? "Try another name or email." : "Add a client, then connect their review projects."}</p></div>}</div>}
